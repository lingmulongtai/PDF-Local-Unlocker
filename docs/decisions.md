...
## 2026-05-28 - ZIPなし一括ダウンロードは個別ダウンロード連続実行にする

### 背景

ユーザーが圧縮なしでも複数PDFをまとめて取得できる選択肢を求めている。ブラウザでは任意フォルダへの直接保存やディレクトリ一括書き込みは標準的に使えない。

### 選択肢

1. ZIPだけを提供する
2. 成功PDFを個別ダウンロードとして連続実行する
3. File System Access APIで保存先フォルダに書き込む

### 採用した案

`Download PDFs` ボタンで成功PDFを個別ダウンロードとして連続実行する。

### 理由

- ZIP化せず、PDFファイルそのものを取得できる
- GitHub Pagesで追加権限なしに動く
- File System Access API非対応ブラウザでも基本動作できる
- 既存の個別ダウンロード処理を安全に再利用できる

### 採用しなかった案と理由

- ZIPだけでは「圧縮なし」の要望に応えにくい
- File System Access APIはブラウザ差が大きく、MVPの対象を狭める

### 今後見直す条件

- 複数ダウンロードが主要ブラウザで強くブロックされる場合
- 保存先フォルダ選択を明示的に提供したくなった場合

## 2026-05-28 - PDFプレビューはObject URLとiframeで行う

### 背景

PDFの内容をアップロードせずに確認したい。プレビューでもPDF bytesを外部送信しない前提を守る必要がある。

### 選択肢

1. Object URLを作り、ブラウザ内PDFビューアに任せる
2. PDF.jsを追加して独自ビューアを作る
3. プレビューを新しいタブで開く

### 採用した案

Object URLを作成し、アプリ内モーダルの `iframe` で表示する。

### 理由

- PDFを外部送信しない
- 追加ライブラリなしで実装できる
- 解除済みBlobがある場合は解除済みPDFを、まだない場合は元PDFを表示できる
- モーダルを閉じるとObject URLを破棄できる

### 採用しなかった案と理由

- PDF.jsは高機能だが依存と実装量が増える
- 新しいタブは操作文脈が分かれ、Object URL管理もしづらい

### 今後見直す条件

- ブラウザ内PDFビューア非対応環境が多い場合
- ページサムネイルやページ指定プレビューが必要になった場合
