...
- ZIPにまとめず、解除済みPDFを個別ファイルとして連続ダウンロードする `Download PDFs` を追加
- 各PDF行からブラウザ内PDFプレビューを開けるようにした
...
- 成功ファイルをZIPなしで個別PDFとして一括ダウンロードする
- 投入済みPDFまたは解除済みPDFをプレビューする
- 処理中にキャンセルする
...
- ブラウザ設定によっては、ZIPなしの一括ダウンロード時に複数ダウンロード許可が必要になる
- ブラウザ内PDFビューアが無効な環境ではプレビューが表示されない場合がある
...
8. 複数ダウンロードとPDFプレビューを主要ブラウザで確認する
