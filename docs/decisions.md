...
## 2026-05-28 - パスワードなしPDFはAlready unlockedとして扱う

### 背景

ユーザーがバッチでPDFを投入すると、パスワード付きPDFとパスワードなしPDFが混在する可能性がある。パスワードなしPDFを単に失敗扱いにすると、アプリが壊れているように見える。

### 選択肢

1. パスワードなしPDFをエラーにする
2. qpdfで暗号化状態を確認し、パスワードなしPDFは成功系の別状態にする
3. 投入時点でPDF.jsなどを使って暗号化状態を事前判定する

### 採用した案

qpdfの `--show-encryption` をWorker内で実行し、`File is not encrypted` の場合は `Already unlocked` として扱う。

### 理由

- qpdf処理の中で判定でき、別ライブラリを増やさずに済む
- パスワード未入力でもパスワードなしPDFは処理できる
- 元PDFをそのままBlob化し、個別ダウンロードとZIP対象にできる
- パスワード付きPDFでパスワード未入力の場合だけ `Password required` と出せる

### 採用しなかった案と理由

- エラー扱いはユーザー体験が悪く、混在バッチに弱い
- PDF.jsによる事前判定は依存と責務が増えるため、現段階では不要

### 今後見直す条件

- qpdfの暗号化判定だけでは分類できないPDFが見つかった場合
- 投入直後に暗号化状態を表示したくなった場合
